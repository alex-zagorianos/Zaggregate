...
**🟡 REMAINING:** (1) **live coverage baseline** — the lift-gates prove improvement on fixtures; a real area number needs a live measurement run (network/sources), best done interactively. (2) WS-3 undo's second-precision batch grouping is fine for the manual round-trip but would only partially undo a batch straddling a wall-clock second — a `batch_id` is the clean follow-up. (3) carry-over from Session 12: `py build_package.py` exe build + manual GUI launch; docx title-line decision; optional first-run wizard.

## Git

- HEAD **`228b013`** on `master`, **pushed to `origin/master`** (even; repo confirmed private). Working tree clean; only `master` branch + worktree remain.
- Remote: `git@github.com:alex-zagorianos/Job-Program.git`
- Full suite: **490 passing** (`py -m pytest -q`, ~12s). Python command: `py`.
