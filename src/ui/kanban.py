...
# Re-export the Tk-free core so `kanban.X` keeps resolving for every existing
# caller and test (COLUMNS, forward_targets, days_in_stage, days_label,
# group_by_status). The pure funnel/day math lives in ui/kanban_core.py so the
# web API can reuse it without importing tkinter (S36 source_keys_core pattern).
from ui.kanban_core import (  # noqa: F401  (re-exported public surface)
    COLUMNS,
    forward_targets,
    days_in_stage,
    days_label,
